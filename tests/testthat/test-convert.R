test_that("can register convert methods", {
  ...
  obj <- converttest()
  expect_equal(convert(obj, character), "c")
  expect_equal(convert(obj, integer), "i")

  # Errors if none found
  expect_snapshot(convert(obj, double), error = TRUE)
})
...
test_that("doesn't convert to subclass", {
  ...
  expect_error(convert(integer, converttest2), "Can't find method")
})
...
describe("fallback convert", {
  ...
  it("can convert to S3 class", {
    ...
    obj <- convert(factor2(1, "x", x = 1), S3_factor)
  ...
  it("can convert to base type", {
    ...
    obj <- convert(character2("x", x = 1), character)
