...
test_that("S4 method registration expands S7 unions to one method per class", {
...
test_that("S4 method registration on class_double catches actual doubles", {
...
test_that("errors when unregistering from an S4 generic", {
  ...
  defer(suppressMessages(methods::removeGeneric("removeS4")))
  S4foo := new_class(package = NULL)
  S4_register(S4foo)
  defer(S4_remove_classes("S4foo"))
