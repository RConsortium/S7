...
[`UseMethod()`](https://rdrr.io/r/base/UseMethod.html) look for the
methods? As of R 4.0.0, it looks in the following three places:

- The method table is a special environment `.__S3MethodsTable__.` found
  in the environment where the generic is defined.

- The chain of environments starting at the
  [`parent.frame()`](https://rdrr.io/r/base/sys.parent.html) of the call
  to generic, ending at the global environment.
...
lookup for S7 generics never needs to look in the parent frame.

`method<-` is likely to start as a shim around
[`.S3method()`](https://rdrr.io/r/base/S3method.html) but we may want to
consider a separate `.__S7MethodsTable__.`. This could use a new data
structure that resolves generic/class ambiguity
(e.g. `all.equal.data.frame()`). Methods for S7 classes defined on an S3
...
Method lookup would be cached for performance, so that it is only
...
[`UseMethod()`](https://rdrr.io/r/base/UseMethod.html) does not work
like a regular function call but instead:
...
- Can we eliminate the special behaviour and make it just like a regular
  function call? Presumably easier than changing dispatch rules because
  we’ll call a function other than
  [`UseMethod()`](https://rdrr.io/r/base/UseMethod.html).

- Need to make precise how arguments are passed to the method.
  [`plot()`](https://rdrr.io/r/graphics/plot.default.html) at least
...
i.e. how does [`NextMethod()`](https://rdrr.io/r/base/UseMethod.html)
work: currently most state recorded in special variables like
`.Generic`, etc.
...
When [`UseMethod()`](https://rdrr.io/r/base/UseMethod.html) receives an
object without a `class` attribute, it uses the **implicit** class, as
provided by [`.class2()`](https://rdrr.io/r/base/class.html). This is
...
Special dispatch? [`c()`](https://rdrr.io/r/base/c.html),
[`cbind()`](https://rdrr.io/r/base/cbind.html),
[`rbind()`](https://rdrr.io/r/base/cbind.html) (+ `cbind2()` and
`rbind2()`) — iterated double dispatch. Need to describe in more detail
so we have a more solid assessment of what S7 might need.ez
