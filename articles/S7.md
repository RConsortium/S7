...
#>  $ age: <integer> or <double> = integer(0)
```
...
lola@age <- "twelve"
#> Error in `<Dog>@age`:
#> ! <Dog>@age must be <integer> or <double>, not <character>
```
...
speak(1)
#> Error:
#> ! Can't find method for `speak(<double>)`.
```

## Named bindings

So far we’ve typed the name of each new class and generic twice: once as
the name of the variable that we assign to, and once as the `name`
argument. Wouldn’t it be nice to eliminate that duplication? That’s the
job of the `:=` operator (the “bind” operator): it uses the name of the
variable on the left-hand side as the `name` argument of the call on the
right-hand side, so `Foo := new_class()` is equivalent to
`Foo <- new_class(name = "Foo")`.

That means we could have defined `Dog` and `speak()` like this:

``` r

Dog := new_class(properties = list(
  name = class_character,
  age = class_numeric
))
Dog
...
We’ll use `:=` in the rest of this vignette, and throughout the rest of
S7’s documentation.
...
Cat := new_class(parent = Pet)
Dog := new_class(parent = Pet)
