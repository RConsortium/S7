...
#define early_return_if_null(obj) \
  if (obj == R_NilValue)          \
  {                               \
    *validate = TRUE;             \
    return value;                 \
...
SEXP prop_val_(SEXP object, SEXP name, Rboolean *validate) {
  SEXP value = Rf_getAttrib(object, name);
  if (value != R_NilValue) {
    // value was accessed as an attr, need to validate to make sure
    // the attr is actually a known class property
    *validate = TRUE;
    return value;
  }

  SEXP S7_class_obj = Rf_getAttrib(object, Rf_install("S7_class"));
  early_return_if_null(S7_class_obj);

  SEXP properties = Rf_getAttrib(S7_class_obj, Rf_install("properties"));
  early_return_if_null(properties);

  SEXP S7_property_obj = extract_name(properties, CHAR(STRING_ELT(name, 0)));
  early_return_if_null(S7_property_obj);

  SEXP getter = extract_name(S7_property_obj, "getter");
  early_return_if_null(getter);

  if (TYPEOF(getter) == CLOSXP)
    value = Rf_eval(Rf_lang2(getter, object), R_GlobalEnv);
  else
    *validate = TRUE;

  return value;
}
#undef early_return_if_null

SEXP prop_(SEXP object, SEXP name) {
  Rboolean validate = FALSE;
  SEXP value = prop_val_(object, name, &validate);
  if (validate == FALSE)
    return value;
  ...
  static SEXP ns = NULL;
  if(ns == NULL)
    ns = Rf_findVarInFrame(R_NamespaceRegistry, Rf_install("S7"));

  static SEXP validate_prop_access = NULL;
  if(validate_prop_access == NULL)
    validate_prop_access = Rf_findVarInFrame(ns, Rf_install("validate_prop_access"));


  Rf_eval(Rf_lang3(validate_prop_access, object, name), ns);
  UNPROTECT(1);
  return value;
}
