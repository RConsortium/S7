...
new_class <- function(
  ...
  check_prop_names(parent, new_props)
...
check_prop_names <- function(parent, properties, error_call = sys.call(-1L)) {
  forbidden <- c("names", "dim", "dimnames", "class", "tsp",
                 "comment", "row.names",
                 if ("factor" %in% class_dispatch(parent)) "levels")
