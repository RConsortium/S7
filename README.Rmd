...
text <- new_class("text", parent = chr, constructor = function(text) new_object(.data = text))
...
method(bar, list(chr, num)) <- function(x, y, ...) paste0("foo-", x, ":", y)
...
```{r}
method(bar, list(text, num)) <- function(x, y, ...) {
  res <- next_method()(x, y)
  paste0("2 ", res)
}
...
method(subset2, df) <- function(x, subset = NULL, select = NULL, drop = FALSE, ...) {
  e <- substitute(subset)
...
method(foo, list(text, num)) <- function(x, y, ...) paste0("foo-", x, ": ", y)
```
