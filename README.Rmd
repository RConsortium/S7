...
[![Codecov test coverage](https://codecov.io/gh/RConsortium/S7/graph/badge.svg)](https://app.codecov.io/gh/RConsortium/S7)
<!-- badges: end -->
...
Use `new_class()` to define a class, with the `:=` operator supplying its name from the variable that you assign the result to:
...
Use `new_generic()` to create a new generic: it needs the generic's name (again supplied with `:=`, and used in error messages) and the arguments used for dispatch.
The optional `fun` argument supplies the body of the generic.
