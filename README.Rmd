...
Dispatch currently uses a `.r7_methods` table defined in the environment of the generic. This table is a nested set of hashed environments based on the classes of the methods. e.g.
...
- Objects
  ...
  - [x] - Additional attributes storing properties defined by the class, accessible with `@/prop()`.
- Classes
...
- Using `@` vs `$` for property access, `@` triggers R CMD check NOTES, needs `utils::globalVariables()`, people are used to using `$` for lists and data.frames
- What should `prop()` return if the property doesn’t exist?
- Returning NULL / character in validator vs throwing an error?
- Opt-out property validation, potentially fatal to performance, maybe default to implicitly off in constructors?
