...
So far we've typed the name of each new class and generic twice: once as the name of the variable that we assign to, and once as the `name` argument. 
Wouldn't it be nice to eliminate that duplication?
That's the job of the `:=` operator (the "bind" operator): it uses the name of the variable on the left-hand side as the `name` argument of the call on the right-hand side, so `Foo := new_class()` is equivalent to `Foo <- new_class(name = "Foo")`.
...
We'll use `:=` in the rest of this vignette, and throughout the rest of S7's documentation.
