...
Let's explore the idea with a real function, `intersect()`, which returns the elements common to two vectors.
Base R's `intersect()` isn't a generic, so it doesn't know how to combine vectors with custom classes.
It would be nice if it were a double-dispatch generic, because most of the time you only want to intersect two vectors of the same type, so you only need to implement methods where `x` and `y` have the same class:

```{r}
intersect := new_generic(c("x", "y"))

method(intersect, list(class_Date, class_Date)) <- function(x, y) {
  .Date(base::intersect(unclass(x), unclass(y)))
}
method(intersect, list(class_POSIXct, class_POSIXct)) <- function(x, y) {
  .POSIXct(base::intersect(unclass(x), unclass(y)), tz = attr(x, "tzone"))
}
```

This implementation means we get the behavior we want when an intersect method exists:

```{r}
weekdays <- as.Date(c("2026-06-15", "2026-06-16", "2026-06-17"))
holidays <- as.Date(c("2026-06-17", "2026-07-04"))
intersect(weekdays, holidays)
```

And a nice error when it doesn't:

```{r, error = TRUE}
intersect(weekdays, 1:3)
```

Occasionally you _do_ want to handle a mix of types, and this is also easy with a double-dispatch generic.
For example, we could intersect a date with a date-time by promoting the date to a date-time and reusing the method above:

```{r}
method(intersect, list(class_Date, class_POSIXct)) <- function(x, y) {
  intersect(as.POSIXct(x, tz = attr(y, "tzone")), y)
}
method(intersect, list(class_POSIXct, class_Date)) <- function(x, y) {
  intersect(x, as.POSIXct(y, tz = attr(x, "tzone")))
}

dt <- as.POSIXct(c("2026-06-15", "2026-06-15 17:00"), tz = "UTC")
intersect(weekdays, dt)
intersect(dt, weekdays)
```

Note that it's important to ensure that the two cases are symmetric, but there's no way for S7 to validate this.
