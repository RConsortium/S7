...
## Using R7 in a package

If you are using R7 in a package _and_ you want your package to maintain
compatibility with R versions prior to 4.3.0, there are a few things to be aware of. 

#### `@`

`` base::`@` `` in R version prior R 4.3 does not resolve R7 properties.

Workarounds:

-  use `R7::prop()` instead of `@`.
-  Alternatively, make an R7-aware `@` available to your package 
   in `.onLoad()` and `.onAttach()` like this: 
``` r
# enable usage of <R7_object>@name in package code
`@` <- NULL
.onLoad <- function(libname, pkgname) {
  `@` <<- get("@", asNamespace("R7"))
}

# enable usage of <R7_object>@name in user code
