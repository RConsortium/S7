...
S7 is new, so few people have used it in a package yet; this means that this vignette is incomplete, and we'd love your help to make it better.
...
If you are using S7 in a package *and* you want your package to work in versions of R older than 4.3.0, then you need to be aware that in these versions of R `@` only works with S4 objects.
There are two workarounds.
The easiest and least convenient workaround is to just `prop()` instead of `@`.
Otherwise, you can make an S7-aware `@` available to your package with this custom `NAMESPACE` directive:
