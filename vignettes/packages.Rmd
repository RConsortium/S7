...
Next, create a `zzz.R` with a `.onLoad()` that calls `methods_register()` and a top-level call to `S7_on_build()`:
...
`methods_register()` is S7's way of registering methods, rather than using export directives in your `NAMESPACE` like S3 and S4 do.
