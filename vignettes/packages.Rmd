...
Conversely, you may want to register a method for one of your own generics, dispatching on an S7 class from a suggested package. Use `new_external_class()` to refer to the class by name:
...
`new_external_class()` is only for S7 classes. For S3 classes from suggested packages, use `new_S3_class()`:

```{r, eval = FALSE}
# In your package
ggplot <- new_S3_class("ggplot")
