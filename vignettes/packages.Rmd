...
A class can't refer to itself directly in its own property definitions, because the class object doesn't exist yet:

```{r, eval = FALSE}
tree <- new_class("tree", properties = list(child = tree))
#> Error in as_properties(properties): object 'tree' not found
```

Use `new_external_class()` to refer to the class by name instead. The name is resolved when the property is accessed, by which time the class is fully defined:
...
tree(label = "outer", child = tree(label = "inner"))
```

The same approach works for mutually recursive classes (`A` references `B` and `B` references `A`), and for classes defined in different files where the file load order would otherwise force a forward reference.
