...
The formal argument of the generic and methods must agree.
This means that:
...
Above, you saw that the standard generic uses `…`.
...
Occasionaly it's useful to generic a generic without `…` because such functions have a useful property: if a call succeeds for one type of input, it will succeed for any type of input.
...
Most calls to `new_generic()` will only use the first two arguments (the name of the generic and the argument (or arguments) to dispatch on), and the body of the generic is automatically generated.
We can use `R7_data()` to extract the underlying function to see what's happening what is generarted:
...
This automatically generated body has two important features:

-   The arguments include `…`, which means that the methods can accept additional arguments.
-   The body of the generic calls `method_call()`, the function that powers method dispatch.

We'll first discuss how `…` comes into play, introducing the idea of generic-method agreement, and then go onto explore situations where you might want to customize the body of the generic.
...
Forgetting the argument or using a different default value will generate a warning.
