...
The two systems are, unfortunately, largely incompatible; this fragmentation in the R community and R sources is undesirable.
...
This blog post outlines four patches that have been integrated into base R to facilitate the existence of S7 as a standalone R package.
Since S7 is built on top of S3, the new capabilities are available not only to S7 but also to all packages built on top of S3.
...
S3 only supports dispatching generics on their first argument, while S4 supports dispatching generics on an arbitrary number of arguments.
S7 uses S3 dispatch, but also implements its own double dispatch (i.e. multiple dispatch on exactly two arguments).
To enable both S3 support and multiple dispatch, dispatch in S7 is performed in two stages: S3 generics first dispatch on the first method to an S7 aware method, which then invokes S7-internal dispatch routines that support multiple dispatch.
These are implementation details which, when everything works, are hidden from S7 users.
The complication which motivated the addition of a new S3 generic is the Ops Group.
...
Before R 4.3.0, if the two objects had different methods for the generic, R would signal a warning and use neither.
...
Because S7 implements its own generic dispatch mechanism, the `chooseOpsMethod()` method for S7 object always returns `TRUE`.
...
For example, the `+` operator can now select the appropriate method if methods for TensorFlow tensors and Python objects generally are both be implemented.
Previously, the restriction of only one of the arguments implementing an S3 method required that the second argument in an Ops call not be an S3 class.
...
Though S7 uses S3 dispatch, the class name used for S3 dispatch is considered an implementation detail (it is non-syntatic and mangled to minimize the potential for conflicts, and not intended to be user-facing).
...
To enable this, beginning with R 4.3.0 `base::inherits()` now accepts arbitrary objects for the second argument: if the second argument is an S3 object, the new `nameOfClass()` S3 generic is invoked to resolve the appropriate class name.
...
Prior to R 4.3.0, `@` could be used only with S4 objects.
Beginning with R 4.3.0, this operator can now perform dispatch S3 dispatch as well.
...
The ability to have formally defined properties is one of the benefits of S4 and S7; it enables collaborative work at a greater scale than the ad-hoc conventions of S3 classes, and having a dedicated operator for property access in S7 is desirable.
...
Similar to `@`, the matrix multiplication operator prior to R 4.3.0 only worked with S4 objects or base R objects.
Now, S3 methods for this operator can be defined as well.

Because `%*%` is an infix operator, it was desirable for it to be able to dispatch on either the first or second argument, like members of the `Ops` group.
However, including `%*%` in the `Ops` group, while perhaps expedient, would not be strictly correct.
Matrix multiplication has different semantics from the other Ops operators, in particular with regards to broadcast semantics---the length of the returned object from `a + b` cannot be expected to be the same as that from `a %*% b`.
For this reason, a new generics Group was created, `matrixOps`.
Presently, the sole member of the group is `%*%`, though in the future, `crossprod()` and `tcrossprod()` are expected to join the group.
This permits for `Ops`-like dispatch behavior (including the new `chooseOpsMethod()`), while also being backwards compatible and a minimally invasive change to the R sources.
The addition of the `"matrixOps"` group is strictly a narrow backwards-compatible change, intended to enable existing operators to work with S3 and S7.
