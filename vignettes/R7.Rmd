...
Sometimes you want to implement a method in terms of behavior of its superclass.
You can access the method for the parent by using `cast_next()`:

```{r}
method(bar, list(text, "double")) <- function(x, y, ...) {
  res <- bar(cast_next(x), y)
  paste0("2 ", res)
}
```
