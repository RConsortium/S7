...
Why did we decide another system (S7) was necessary?
This vignette will discuss some of the motivations, focussing on the aspects of S3 and S4 that have been found to be particularly challenging in practice.
...
-   S3 is very informal, meaning that there's no formal definition of a class.
    This makes it impossible to know exactly which properties an object should or could possess, or even what it's parent class should be.
...
-   Properties of an S3 class are usually stored in attributes, but, by default, `attr()` does partial matching, which can lead to bugs that are hard to diagnose.
    Additionally, `attr()` returns `NULL` if an attribute exists, so misspelling an attribute can lead to subtle bugs.
...
-   `NextMethod()` is unpredictable you can't tell exactly what method will be called by only reading the code; you instead need to know both the complete class hierarchy and what other methods are currently registered (and loading a package might change those methods).
...
-   In S4, slots are generally thought of as implementation detail that should not be directly accessed by the end-user.
    This leads to two problems.
    Firstly, implementing an S4 class often also requires a plethora of accessor functions that are a thin wrapper around `@` or `@<-`.
    Secondly, users know about `@` and often use it even though they're not supposed to.
...
-   S4's method dispatch uses a thoughtful system of weights to pick the best method when there is ambiguity.
