...
-   Non R7 classes
    -   `as_class()` centralises all handling of classes (base types, S3, S4, and R7)
    -   Strings now only refer to base types (previously used for S3 + R7 class in parent frame) `new_class( parent = "function")`
    -   Numeric, atomic, and vector "types" as R7 unions.
    ...
    -   New `s3_class()` to define S3 classes.
        -   <https://github.com/RConsortium/OOP-WG/pull/148> makes it possible for R7 class to extend an S3 class
        -   Should we export the S3 class definitions for common base S3 classes? And then provide with `base_factor` etc.
    -   S4 unions converted to R7 unions
-   Method registration
    -   Clarify difference between registration and dispatch. Will flow into refactoring of C code in near future.
    -   Dispatch is base/S3/S4/R7 class with R7 generic.
    -   Registration is base/S3/S4/R7 class with R7 generic AND R7 class with S3/S4 generic.
