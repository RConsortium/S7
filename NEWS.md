...
* `method<-` now registers a base-visible S3 method (a bridge into S7 dispatch) when you register a base operator method (e.g. `+`) with an S3 class in its signature, so dispatch works even when the S3 object meets a non-S7 value, e.g. `foo + 10` where `foo` has an informal S3 class. Any existing operator method for that class (such as `Ops.factor`) is preserved for signatures you haven't overridden (#544).
