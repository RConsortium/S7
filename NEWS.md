...
* `new_property()` now accepts a `setter` that takes `self`, `name`, and `value` (in addition to the existing `self`, `value` form), making it easy to reuse the same property definition for differently-named properties within a class (@teunbrand, #552).
