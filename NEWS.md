...
* `new_object()` is now substantially faster because each class caches its own name and dispatch vector, and class type detection has moved to C. These changes make construction 1.4x faster for a class that directly extends `S7_object`, rising to 2.4x faster for a class with 10 ancestors; `S7_inherits()` is 1.5x faster and `super()` is 2.1x faster. On R 4.3 and later, internal property access now uses base `@` directly, reducing constructor overhead further (#723).
