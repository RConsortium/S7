...
* `new_class()` now stashes the class in its constructor's environment, so `new_object()` can find it without `sys.function()`, which deep-copies the class and all of its ancestors. Every instance of a class now shares a single class object instead of holding a private copy, dramatically reducing the memory used by S7 objects: an instance of a class with five ancestors drops from roughly 19 kB to under 300 bytes, and the per-instance cost no longer grows with the depth of the class hierarchy (#723).
