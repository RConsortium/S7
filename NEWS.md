...
* The `:=` operator now stays ahead of other attached packages that export `:=`, such as rlang or data.table, without emitting attach-time masking messages. Masking messages for other conflicts are still emitted. S7 also declares its expected masks via `conflictRules()`, so once its namespace is loaded it can attach under `options(conflicts.policy = "strict")`.
