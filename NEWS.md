...
* The class object that S7 stores on each instance now lives in the `_S7_class` attribute (previously `S7_class`), moving it into the `_`-prefixed namespace reserved for S7 internals so it can't collide with a user-defined property. Objects serialised by an older version of S7 (e.g. baked into another package's lazy-load database) must be rebuilt to be recognised by this version.
