...
* `new_object()` is faster again, cutting the fixed bookkeeping cost paid on every constructor call: the names of properties with custom setters are now computed once when the class is created, constructions that supply no such properties skip the setter machinery entirely, and the class's dispatch vector is reused rather than rebuilt. Construction is 1.2x faster for a class that directly extends `S7_object` and 1.4x faster for a class with no properties (#723).
