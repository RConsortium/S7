...
* `S7_classes()` and `S7_generics()` list the S7 classes / generics defined in a given environment (default: the caller's environment); pass `asNamespace("pkg")` to inspect a specific package (#335).
