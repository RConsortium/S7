...
* `new_object()` no longer copies an S7 class each time a default or custom constructor creates an object. New objects instead store an external-pointer class reference, so class definitions are not serialized with their instances. After restoration, the reference lazily resolves the current class definition and validates the object once before caching it. Constructors created by older versions of S7 continue to work through the previous fallback (#742).
